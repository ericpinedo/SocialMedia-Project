...
class FeedVC: UIViewController, UITableViewDataSource, UITableViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
    ...
    @IBAction func makePost(sender: AnyObject) {
        ...
        if let txt = postField.text where txt != "" {
            ...
            if let img = postImg.image where img != cameraImage {
                let imgData = UIImageJPEGRepresentation(img, 0.2)
                let imgPath = "\(NSDate.timeIntervalSinceReferenceDate())"
                let metadata = FIRStorageMetadata()
                metadata.contentType = "image/jpg"
                DataService.ds.REF_IMAGES.child(imgPath).putData(imgData!, metadata: metadata, completion: {
                    metadata, error in
                    if error != nil{
...
//                let imgData = UIImageJPEGRepresentation(img, 0.2)!
//                
//                let keyData = "17BDFLQU2780e1acb03457149fdc5fb5b9755cd4".dataUsingEncoding(NSUTF8StringEncoding)!
